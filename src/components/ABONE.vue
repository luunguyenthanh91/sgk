<template>
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups_1" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 1</span>
    ...
    <section class="page" v-for="(questions, qsIdx) in groups_2" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 1</span>
        <span class="title-sgk">Abacus Arithmetic</span>
        <span class="page-sgk"></span>
      ...
      <div class="header-page align-end footer-bar">
        <span class="page-sgk">
          <img
            v-if="(questions.pageIndex + groups_1.length) % 2 == 0"
            src="../assets/logo_footer.png"
          />
        </span>
        <span class="page-sgk">{{ questions.pageIndex + groups_1.length }}</span>
        <span class="page-sgk">
          <img
            v-if="(questions.pageIndex + groups_1.length) % 2 == 1"
            src="../assets/logo_footer.png"
          />
        </span>
      </div>
    </section>
    <section class="page" v-for="(questions, qsIdx) in groups_3" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 1</span>
        <span class="title-sgk">Abacus Arithmetic</span>
        <span class="page-sgk"></span>
      ...
      <div class="header-page align-end footer-bar">
        <span class="page-sgk">
          <img
            v-if="(questions.pageIndex + groups_1.length + groups_2.length) % 2 == 0"
            src="../assets/logo_footer.png"
          />
        </span>
        <span class="page-sgk">{{
          questions.pageIndex + groups_1.length + groups_2.length
        }}</span>
        <span class="page-sgk">
          <img
            v-if="(questions.pageIndex + groups_1.length + groups_2.length) % 2 == 1"
            src="../assets/logo_footer.png"
          />
        </span>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      groups_1: { type: Array, required: true },
      groups_2: { type: Array, required: true },
      groups_3: { type: Array, required: true },
      answerPages: { type: Array, required: true },
    };
  },
  computed: {
    answerPagesMatrix() {
  ...
  async mounted() {
    const jsonPath = "/AB1.json";
    // --- Thay thế toàn bộ block try { ... } catch { ... } trong mounted() bằng đoạn sau ---
    try {
      const loaded = await this.tryLoadJson(jsonPath);

      // Nếu không load được -> fallback tạo mới (giữ logic cũ trong catch)
      ...
      if (
        ...
        this.groups_1 = this.nestPages(q3, 20, 5);
        this.groups_2 = this.nestPages(q4, 20, 5);
        this.groups_3 = this.nestPages(q5, 20, 5);

        const all = [...q3, ...q4, ...q5];
        this.groups = this.nestPages(all, 20, 5);

        // Tạo answer pages từ this.groups (flatten)
        this.answerPages = this.makeAnswerPages(3);
        return;
      }

      // CASE B: file là 3 mảng trực tiếp [q3,q4,q5] (giả sử)
      if (Array.isArray(loaded) && loaded.length === 3 && loaded.every(Array.isArray)) {
        const [q3, q4, q5] = loaded;
      ...
      // CASE C: nếu loaded là 1 mảng *toàn bộ các câu* (all questions)
      if (
        Array.isArray(loaded) &&
        loaded.length &&
        loaded[0] &&
        typeof loaded[0] === "object" &&
        Array.isArray(loaded[0].numbers)
      ) {
        const all = loaded;
        // chia thành groups theo cấu hình cũ
        this.groups = this.nestPages(all, 20, 5);

        // nếu muốn chia riêng q3/q4/q5 bạn phải lưu thông tin loại trong file; ta không giả định ở đây
        this.groups_1 = this.groups; // tạm map để template không lỗi — bạn có thể thay đổi theo nhu cầu
        this.groups_2 = [];
        this.groups_3 = [];
      ...
      // Nếu file có dạng khác không lường trước -> fallback tạo mới
      throw new Error("unsupported-format");
    } catch (err) {
      // Fallback: tạo mới nếu load thất bại hoặc format không đúng (giữ nguyên logic cũ)
      const q3 = this.generateQuestions(400, 3, 3); // 400 câu 3 số
      const q4 = this.generateQuestions(440, 4, 4); // 440 câu 4 số
      const q5 = this.generateQuestions(500, 5, 5); // 500 câu 5 số
      ...
      // Chia trang 20 câu, reset id 1→20
      this.groups_1 = this.nestPages(q3, 20, 5);
      this.groups_2 = this.nestPages(q4, 20, 5);
      this.groups_3 = this.nestPages(q5, 20, 5);
      ...
      this.saveJsonForBrowserDownload(dataJson, "AB1.json");
  ...
  methods: {
    saveJsonForBrowserDownload(obj, filename = "AB1.json") {
      try {
