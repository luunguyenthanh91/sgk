  ...
  <div ref="exportRoot">
    <section class="page" v-for="(pg, idx) in groups" :key="idx">
      <div class="header-page">
        <span class="title-sgk">Level 10</span>
  ...
  async mounted() {
    ...
    try {
      ...
      if (loaded && Array.isArray(loaded)) {
        ...
        // Rebuild mul/div answer maps from saved muldiv pages (if any)
        ...
        // No valid JSON -> generate groups (keeps your existing generation logic)
        const TOTAL_PAGES = 64; // giữ nguyên hoặc đổi
        ...
        // tạo các trang chẵn (mỗi trang 15 câu: 5+5+5)
        for (let page = 0; page < EVEN_COUNT; page++) {
          // 5 câu hundreds
          for (let k = 0; k < 5; k++) {
            const s = this.genSequence({ count: 8, scale: "hundreds", negCount: 3 });
            itemsEven.push({
              numbers: s.numbers,
              sum: s.sum,
              type: 6,
          ...
          // 5 câu thousands
          for (let k = 0; k < 5; k++) {
            const s = this.genSequence({ count: 6, scale: "thousands", negCount: 2 });
            itemsEven.push({
              numbers: s.numbers,
              sum: s.sum,
              type: 6,
          ...
          // 5 câu decimal (vẫn dùng decMix như bạn viết)
          for (let k = 0; k < 5; k++) {
            const tensCount = this.randInt(2, 8);
            const hundredsCount = 10 - tensCount;
            const s = this.genSequence({
              count: 10,
              scale: "decimal",
              negCount: 3,
              decScale: "tens",
              decMix: { tens: tensCount, hundreds: hundredsCount },
            });
            itemsEven.push({
              numbers: s.numbers,
              sum: s.sum,
        ...
        // Merge odd + even pages interleaved
        const merged = [];
        ...
        // Save generated groups to AB10.json so next load will restore same data
        try {
          this.saveJsonForBrowserDownload(this.groups, "AB10.json");
        } catch (saveErr) {
          console.error("Failed to auto-save AB10.json:", saveErr);
        }

        // Rebuild mul/div maps for safety (makeMulDivPage already set them when created,
        // but rebuild ensures consistency after merging)
        for (const pg of this.groups) {
          if (
            pg &&
            pg.mode === "muldiv40" &&
      ...
      // Fetch/json error -> fallback to generate + save (same as above)
      console.error("Error loading AB10.json, generating new groups:", err);

      const TOTAL_PAGES = 64;
      const ODD_COUNT = Math.ceil(TOTAL_PAGES / 2);
      const EVEN_COUNT = Math.floor(TOTAL_PAGES / 2);
      const oddPages = [];
      for (let i = 0; i < ODD_COUNT; i++) oddPages.push(this.makeMulDivPage(2 * i + 1));
      ...
      const merged = [];
      const L2 = Math.max(oddPages.length, evenPages.length);
      for (let i = 0; i < L2; i++) {
        if (i < oddPages.length) merged.push(oddPages[i]);
        if (i < evenPages.length) merged.push(evenPages[i]);
      }
      ...
      try {
        ...
        console.error("Failed to auto-save AB10.json after catch:", saveErr);
      }
    ...
    // finally build answerPages always
    this.answerPages = this.makeAnswerPages(3);
  },

  methods: {
    saveJsonForBrowserDownload(obj, filename = "AB10.json") {
      try {
