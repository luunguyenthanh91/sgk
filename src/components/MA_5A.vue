  ...
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 5A</span>
...
export default {
  ...
  mounted() {
    ...
    for (let page = 0; page < 67; page++) {
      ...
      // 5 câu decimal unit
      for (let i = 0; i < 5; i++) {
        const q = this.generateTwoTensTwoUnitsWith1Minus();
        items.push(q);
      }
    }
  ...
  methods: {
    generateTwoTensTwoUnitsWith1Minus() {
      ...
      for (let t = 0; t < MAX_TRIES; t++) {
        // 1) Sinh 2 tens + 2 units
        const tens = [randTen(), randTen()];
        const units = [randUnit(), randUnit()];

        // 2) Trộn để vị trí 2 số tens hoàn toàn ngẫu nhiên trong 4 vị trí
        const base = this.shuffle([...tens, ...units]);

        // 3) Thử vài phương án gán 1 số âm + kiểm tra ràng buộc
        for (let sTry = 0; sTry < 8; sTry++) {
          // đôi khi reshuffle lại để đa dạng
          const arr = sTry < 4 ? base.slice() : this.shuffle(base);

          const negIdx = Math.floor(Math.random() * 4); // 0..3
          arr[negIdx] = -Math.abs(arr[negIdx]); // đảm bảo đúng 1 số âm

          // 4) Kiểm tra prefix-sum > 0 và tổng trong (10,100)
          let sum = 0,
            ok = true;
          for (const v of arr) {
            sum += v;
      ...
      // Fallback cực hiếm: dựng chuỗi có đệm dương trước rồi thử lại
      // (giữ đúng domain & random tiếp để đảm bảo prefix)
      return this.generateTwoTensTwoUnitsWith1Minus();
    },
