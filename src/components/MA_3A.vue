  ...
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 3A</span>
    ...
    >
      ...
      <main class="sheet-grid" v-if="chunk">
        ...
        <table class="answers-table matrix">
          ...
          <tbody>
            <!-- TRÁI -->
            <tr v-for="qId in ROWS_PER_PAGE" :key="'row-left-' + qId">
        ...
        <table class="answers-table matrix">
          ...
          <tbody>
            <!-- PHẢI -->
            <tr v-for="qId in ROWS_PER_PAGE" :key="'row-right-' + qId">
              <td class="q-index">{{ qId }}</td>
              <td
...
export default {
  ...
  data() {
    return {
      ...
      groups: [],
      answerPages: [],
      negativesPerQuestion: 1, // ⇦ số âm/câu: chỉnh ở đây
    };
  },
  computed: {
    answerPagesMatrix() {
    ...
    answerMap() {
      ...
      for (const page of this.groups || []) {
        const p = page.pageIndex;
        if (!map[p]) map[p] = Object.create(null);
        for (const chunk of page.items) {
          for (const q of chunk) {
            // dùng tổng (hoặc q.sum nếu bạn đã gán sẵn)
            const total = q.sum ?? q.numbers.reduce((a, b) => a + b, 0);
            map[p][q.id] = total; // ✅ lưu TỔNG, không phải hàng chục
          }
        }
      }
      return map;
    ...
    allPagesSorted() {
      return (this.groups || []).map((p) => p.pageIndex).sort((a, b) => a - b);
    },
  ...
  mounted() {
    const q3 = this.generateQuestions(1340, 3, this.negativesPerQuestion);
    this.groups = this.nestPages(q3, 20, 5);
  ...
  methods: {
    ...
    // Sinh bộ câu hỏi
    generateQuestions(count, countNums = 3, negPerQ = 1) {
      const items = [];
      ...
      for (let i = 0; i < count; i++) {
        ...
        while (true) {
          // 1. Sinh dãy toàn dương
          numbers = Array.from({ length: countNums }, (_, idx) =>
            idx === 0 ? this.randDigit1_9() : this.randDigit1_9()
          );

          // 2. Random vị trí đặt số âm (trừ vị trí đầu tiên)
          const positions = [...Array(countNums).keys()].slice(1); // [1, 2,...]
          const negPos = this.pickRandom(positions, negPerQ);
          for (const pos of negPos) {
            numbers[pos] *= -1;
          }

          // 3. Kiểm tra mọi tổng trung gian và tổng cuối đều ≥ 0
          let running = 0,
            valid = true;
          ...
          // Nếu không hợp lệ thì sinh lại
        }

        items.push({
          numbers,
          sum,
          type: "sum_random_negatives",
        });
      }
