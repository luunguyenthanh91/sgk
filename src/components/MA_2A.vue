  ...
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 2A</span>
  ...
  async mounted() {
    ...
    try {
      ...
      if (loaded && Array.isArray(loaded)) {
        ...
        const q3 = this.generateQuestions(1340, 3);
        this.saveJsonForBrowserDownload(q3, "MA2A.json");
        this.groups = this.nestPages(q3, 20, 5);
      }
    } catch (err) {
      const q3 = this.generateQuestions(1340, 3);
    ...
    // tạo trang đáp án từ dữ liệu câu hỏi
    this.answerPages = this.makeAnswerPages(3);
  },
  methods: {
    saveJsonForBrowserDownload(obj, filename = "MA2A.json") {
      try {
    ...
    // Sinh bộ câu hỏi
    generateQuestions(count, countNums = 3) {
      const items = [];
      for (let i = 0; i < count; i++) {
        let numbers, total;

        // 3 chữ số 1..9, tổng phải >= 10
        do {
          numbers = Array.from({ length: countNums }, () => this.randDigit1_9());
          total = numbers.reduce((a, b) => a + b, 0); // 3..27
        } while (total <= 9);
