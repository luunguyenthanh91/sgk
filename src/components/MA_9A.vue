  ...
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 9A</span>
...
export default {
  ...
  mounted() {
    ...
    for (let page = 0; page < 67; page++) {
      // 5 câu hundreds
      for (let i = 0; i < 5; i++) {
        const q = this.generateNineTensWith3Minus();
        items.push(q);
      }

      for (let i = 0; i < 5; i++) {
        const q = this.generateFourHundredsWith1Minus();
        items.push(q);
      }
      for (let i = 0; i < 5; i++) {
        const q = this.generateTwoThousandsFirstPositiveTotalPositive();
        items.push(q);
      }
    }
  ...
  methods: {
    generateTwoThousandsFirstPositiveTotalPositive() {
      // eslint-disable-next-line no-constant-condition
      while (true) {
        const a = Math.floor(Math.random() * 9000) + 1000; // số đầu: luôn dương
        ...
        if (sum >= 0) {
          return {
            numbers: [a, valB],
            sum,
            type: "2thousands_first_positive_total_positive",
            place: "thousands",
          };
        }
      }
    ...
    generateFourHundredsWith1Minus() {
      // eslint-disable-next-line no-constant-condition
      while (true) {
        // 1. Tạo 4 số từ 100–999
        ...
        // 3. Chọn 1 vị trí bất kỳ từ index 1–3 để gán số âm
        const idx = this.shuffle([1, 2, 3])[0];

        const arr = shuffled.map((v, i) => (i === idx ? -v : v));

        // 4. Kiểm tra tổng từng bước > 0
        let sum = 0;
        let valid = true;
        for (const v of arr) {
          sum += v;
        ...
        if (valid /* && sum >= 100 */) {
          return {
            numbers: arr,
            sum,
            type: "4hundreds_1minus_shuffled",
            place: "hundreds",
          };
        }
      }
    ...
    generateNineTensWith3Minus() {
      // eslint-disable-next-line no-constant-condition
      while (true) {
        // 1. Tạo 9 số từ 10–99
        const tens = Array.from({ length: 9 }, () => Math.floor(Math.random() * 90) + 10);

        // 2. Trộn vị trí
        const shuffled = this.shuffle([...tens]);

        // 3. Chọn 3 vị trí bất kỳ (trừ index 0) để gán dấu âm
        const validIdx = [1, 2, 3, 4, 5, 6, 7, 8];
        const negIdx = this.shuffle(validIdx).slice(0, 3);

        const arr = shuffled.map((v, i) => (negIdx.includes(i) ? -v : v));
    ...
    generateTwoHundredsPositiveSum() {
      // eslint-disable-next-line no-constant-condition
      while (true) {
        ...
        // Số thứ hai có thể âm hoặc dương
        const signB = Math.random() < 0.5 ? -1 : 1;
        const valB = b * signB;
        ...
        if (total >= 0) {
          return {
            numbers: [valA, valB],
            sum: total,
            type: "2hundreds_positive_sum_no_neg_first",
            place: "hundreds",
          };
        }
      }
    ...
    generateSevenTensWith3Minus() {
      // eslint-disable-next-line no-constant-condition
      while (true) {
        // 1. Sinh 7 số từ 10–99
        const tens = Array.from({ length: 7 }, () => Math.floor(Math.random() * 90) + 10);

        // 2. Trộn vị trí
        const shuffled = this.shuffle([...tens]);

        // 3. Chọn 3 vị trí để gán dấu âm (không chọn index 0)
        const validIdx = [1, 2, 3, 4, 5, 6];
        const idx = this.shuffle(validIdx);
        const negA = idx[0];
        const negB = idx[1];
        const negC = idx[2];

        const arr = shuffled.map((v, i) =>
          i === negA || i === negB || i === negC ? -v : v
        );
        ...
        // 5. Kiểm tra tổng cuối > 100
        if (ok && s > 100) {
          return {
            numbers: arr,
            sum: s,
            type: "7tens_3minus_shuffled",
            place: "mixed",
          };
        }
      }
    ...
    generateFiveTensWith2Minus() {
      // eslint-disable-next-line no-constant-condition
      while (true) {
        // 1. Tạo 5 số từ 10–99
        const tens = Array.from({ length: 5 }, () => Math.floor(Math.random() * 90) + 10);

        // 2. Trộn random vị trí
        const shuffled = this.shuffle([...tens]);

        // 3. Chọn 2 vị trí khác nhau để gán số âm, cấm index 0
        const validIdx = [1, 2, 3, 4];
        const idx = this.shuffle(validIdx);
        const negA = idx[0];
        const negB = idx[1];

        const arr = shuffled.map((v, i) => (i === negA || i === negB ? -v : v));

        // 4. Kiểm tra tổng từng bước (prefix-sum) > 0
        let s = 0;
        let ok = true;
        for (const v of arr) {
          s += v;
        ...
        if (ok && s > 10) {
          return {
            numbers: arr,
            sum: s,
            type: "5tens_2minus_shuffled",
            place: "mixed",
          };
        }
        // nếu fail thì chạy lại vòng lặp
      }
    },

    // ====== Generic builder: tensCount + unitsCount, exactly 2 negatives, first not negative ======
    generateFourTensThreeUnitsWith2Minus() {
      // eslint-disable-next-line no-constant-condition
      while (true) {
        // 1. Sinh 4 số từ 10–99
        const tens = Array.from({ length: 4 }, () => Math.floor(Math.random() * 90) + 10);
        // 2. Sinh 3 số từ 1–9
        const units = Array.from({ length: 3 }, () => Math.floor(Math.random() * 9) + 1);

        // 3. Gộp và trộn random
        const all = this.shuffle([...tens, ...units]);

        // 4. Chọn 2 vị trí khác nhau để gán dấu âm, nhưng cấm index 0
        const indexes = [...Array(7).keys()].slice(1); // [1,2,3,4,5,6]
        const shuffledIdx = this.shuffle(indexes);
        const negIdxA = shuffledIdx[0];
        const negIdxB = shuffledIdx[1];

        const final = all.map((v, i) => (i === negIdxA || i === negIdxB ? -v : v));

        // 5. Kiểm tra từng bước cộng dồn > 0
        let s = 0;
        let ok = true;
        ...
        if (ok && s > 10) {
          return {
            numbers: final,
            sum: s,
            type: "4tens_3units_2minus_shuffled",
            place: "mixed",
          };
        }
        // Nếu sai điều kiện thì chạy lại vòng lặp
      }
    },
