<template>
  <div ref="exportRoot">
    <section class="page" v-for="(pg, idx) in groups" :key="idx">
      <div class="header-page">
        ...
        <span class="title-sgk">
          <div v-if="idx == 0" class="math-problem-container-v2">
            <img src="../assets/1.png" />
          </div>
          <div v-if="idx == 43" class="math-problem-container-v2">
            <img src="../assets/2.png" />
          </div>
        </span>
        <span v-if="idx < 8" class="title-sgk">&times; {{ idx + 2 }}</span>
        <span v-if="idx >= 8 && idx < 33" class="title-sgk"
          ><i class="fa fa-star"></i
        ></span>
        <span v-if="idx >= 33 && idx < 43" class="title-sgk">&nbsp;</span>
        <span v-if="idx >= 43 && idx < 51" class="title-sgk"
          ><i class="fa fa-star"></i
        ></span>
        <span v-if="idx >= 51" class="title-sgk">&nbsp;</span>
      </div>
          ...
          <table class="md-table">
            <tbody>
              <tr v-for="(q, i) in pg.div" :key="'d-' + i">
                ...
                <td class="expr">
                  {{ convertNumber(q.ab) }} × {{ convertNumber(q.b) }} =
                </td>
              </tr>
            </tbody>
          </table>
    ...
    >
      <div class="header-page">
        <span class="title-sgk">Multiplication</span>
...
export default {
  ...
  async mounted() {
    const jsonPath = "/ABM.json";
    try {
      const loaded = await this.tryLoadJson(jsonPath);

      if (loaded && Array.isArray(loaded)) {
        // Restore saved data exactly
        this.pages = loaded;
        this.groups = loaded;

        // Rebuild mul/div maps from saved muldiv pages (if any)
        for (const pg of this.groups) {
          if (
            pg &&
            pg.mode === "muldiv40" &&
            Array.isArray(pg.mul) &&
            Array.isArray(pg.div)
          ) {
            const mulMap = {};
            const divMap = {};
            pg.mul.forEach((m, i) => {
              mulMap[i + 1] = m.ans ?? Number(m.a) * Number(m.b);
            });
            pg.div.forEach((d, i) => {
              // in your template dq uses ab and b; stored ans should be present or compute
              divMap[i + 1] =
                d.ans ?? Number(d.ab ?? d.dividend ?? 0) * Number(d.b ?? d.divisor ?? 1);
            });
            this.mulAnsMap = { ...this.mulAnsMap, [pg.pageIndex]: mulMap };
            this.divAnsMap = { ...this.divAnsMap, [pg.pageIndex]: divMap };
          }
        }
      } else {
        // No saved JSON -> generate groups (giữ logic hiện tại của bạn)
        ...
        const itemsEven = []; // giữ rỗng như trước (nếu bạn muốn tạo even pages thì thêm logic)
        ...
        // Save to this.pages then trigger browser download as requested
        try {
          this.pages = this.groups;
          this.saveJsonForBrowserDownload(this.pages, "ABM.json");
        } catch (saveErr) {
          console.error("Failed to auto-save ABM.json:", saveErr);
        }

        // Rebuild mul/div maps to be safe (makeMulDivPage already sets them, but ensure consistency)
        for (const pg of this.groups) {
          if (
            pg &&
            pg.mode === "muldiv40" &&
      ...
      console.error("Error loading ABM.json, generating new groups:", err);

      // Fallback: generate same as in else
      const TOTAL_PAGES = 64; // ví dụ
      const ODD_COUNT = Math.ceil(TOTAL_PAGES);
      const oddPages = [];
      for (let i = 0; i < ODD_COUNT; i++) {
      ...
      try {
        ...
        console.error("Failed to auto-save ABM.json after catch:", saveErr);
      }
    ...
    // Always (re)build answerPages if you rely on it
    this.answerPages = this.makeAnswerPages ? this.makeAnswerPages(3) : this.answerPages;
  },

  methods: {
    saveJsonForBrowserDownload(obj, filename = "ABM.json") {
      try {
    ...
    makeMulDivPage(pageIndex) {
      let left_content = "2 digits x 1 digit";
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (pageIndex <= 1) {
          ...
          let ab;
          if (i < 10) {
            // 10 câu đầu: mỗi chữ số ×2 <10 => hàng chục, hàng đơn vị <5
            if (i == 0) {
              a = 34;
            } else {
              const tens = this.randInt(1, 4);
              const ones = this.randInt(1, 4);
              a = tens * 10 + ones;
            }

            const tens_b = this.randInt(5, 9);
            const ones_b = this.randInt(1, 4);
            ab = tens_b * 10 + ones_b;
          } else {
            const tens = this.randInt(1, 4);
            const ones = this.randInt(5, 9);
            a = tens * 10 + ones;

            const tens_b = this.randInt(5, 9);
            const ones_b = this.randInt(5, 9);
            ab = tens_b * 10 + ones_b;
          }
          mq = { a, b: 2, ans: a * 2 };
          dq = { ab, b: 2, ans: ab * 2 };
          ...
          if (i < 10) {
            // 10 câu đầu: mỗi chữ số ×2 <10 => hàng chục, hàng đơn vị <5
            const tens = this.randInt(1, 3);
            const ones = this.randInt(1, 3);
            a = tens * 10 + ones;

            const tens_b = this.randInt(4, 9);
            const ones_b = this.randInt(1, 3);
            ab = tens_b * 10 + ones_b;
          } else {
            const tens = this.randInt(1, 3);
            const ones = this.randInt(4, 9);
            a = tens * 10 + ones;

            const tens_b = this.randInt(4, 9);
            const ones_b = this.randInt(4, 9);
            ab = tens_b * 10 + ones_b;
          }
          mq = { a, b: 3, ans: a * 3 };
          dq = { ab, b: 3, ans: ab * 3 };
          ...
          if (i < 10) {
            // 10 câu đầu: mỗi chữ số ×2 <10 => hàng chục, hàng đơn vị <5
            const tens = this.randInt(1, 2);
            const ones = this.randInt(1, 2);
            a = tens * 10 + ones;

            const tens_b = this.randInt(3, 9);
            const ones_b = this.randInt(1, 2);
            ab = tens_b * 10 + ones_b;
          } else {
            const tens = this.randInt(1, 2);
            const ones = this.randInt(3, 9);
            a = tens * 10 + ones;

            const tens_b = this.randInt(3, 9);
            const ones_b = this.randInt(3, 9);
            ab = tens_b * 10 + ones_b;
          }
          mq = { a, b: 4, ans: a * 4 };
          dq = { ab, b: 4, ans: ab * 4 };
          ...
          if (i < 10) {
            // 10 câu đầu: mỗi chữ số ×2 <10 => hàng chục, hàng đơn vị <5
            const tens = 1;
            const ones = this.randInt(2, 9);
            a = tens * 10 + ones;

            const tens_b = this.randInt(2, 9);
            const ones_b = this.randInt(2, 9);
            ab = tens_b * 10 + ones_b;
          } else {
          ...
          mq = { a, b: pageIndex + 1, ans: a * (pageIndex + 1) };
          dq = { ab, b: pageIndex + 1, ans: ab * (pageIndex + 1) };
          ...
          const maxDigitSafe = (b) => maxDigitMap[b] || 1; // max_digit để tích < 10 (tối đa là 9)

          // max_digit_carry = ceil(10 / b)
          const minDigitCarry = (b) => {
            if (b <= 1) return 2; // 2 * 1 >= 2 (luôn < 10, nên khó tạo carry, dùng 2-9)
            return Math.ceil(10 / b);
          };

          let a; // Số bị nhân cho mq (2 chữ số)
          let ab; // Số bị chia cho dq (2 chữ số)
          let b;

          if (i < 10) {
            b = this.randInt(2, 9);
            const max_d = maxDigitSafe(b);
            const min_d_carry = minDigitCarry(b);

            const tens = this.randInt(1, max_d); // Hàng chục không được là 0
            const ones = this.randInt(1, max_d); // Hàng đơn vị không được là 0
            a = tens * 10 + ones;

            const tens_b = this.randInt(min_d_carry, 9);
            const ones_b = this.randInt(1, max_d);
            ab = tens_b * 10 + ones_b;
          } else {
            // === 1. RANDOM B (Lưu ý: Bị random lại, khác với b của 10 câu đầu) ===
            b = this.randInt(2, 9);

            // Tận dụng lại các hàm trợ giúp
            const max_d = maxDigitSafe(b); // Tối đa để tích < 10 (No Carry)
            const min_d_carry = minDigitCarry(b); // Tối thiểu để tích >= 10 (Carry)

            // Nếu min_d_carry > 9 (tức b=1), thì không thể tạo Carry, ta phải dùng No Carry
            if (min_d_carry > 9) {
              // Trường hợp b=1 (min_d_carry = 10): Phép nhân/chia đơn giản nhất
              const tens = this.randInt(1, 9);
              const ones = this.randInt(1, 9);
              a = tens * 10 + ones;
              ab = a; // ab là thương
            } else {
              // === 2. TẠO a (cho mq: a x b) ===
              // tens * b < 10 VÀ ones * b >= 10
              const tens = this.randInt(1, max_d); // Hàng chục: KHÔNG CARRY
              const ones = this.randInt(min_d_carry, 9); // Hàng đơn vị: CÓ CARRY
              a = tens * 10 + ones;

              // === 3. TẠO ab (cho dq: ab / b) ===
              // tens_b * b >= 10 VÀ ones_b * b >= 10
              const tens_b = this.randInt(min_d_carry, 9); // Hàng chục: CÓ CARRY
              const ones_b = this.randInt(min_d_carry, 9); // Hàng đơn vị: CÓ CARRY
              ab = tens_b * 10 + ones_b; // ab là thương (quotient)
            }
          }

          mq = { a, b, ans: a * b };
          dq = { ab, b, ans: ab * b };
        } else if (pageIndex <= 43) {
          // Trang 1: hàng chục × 2
          let a;
          let ab;
          ...
          const tens_b = this.randInt(1, 9);
          const ones_b = this.randInt(1, 9);
          ab = tens_b * 10 + ones_b;
          ...
          const thour = this.randInt(1, max_d);
          const tens = this.randInt(1, max_d);
          const ones = this.randInt(1, max_d);
          if (i < 10) {
            if (i == 0 && pageIndex == 44) {
              b = 2;
              a = 324;
            } else {
              a = thour * 100 + tens * 10 + ones;
            }

            const thour_b = this.randInt(1, max_d);
            const tens_b = this.randInt(min_d_carry, 9);
            const ones_b = this.randInt(1, max_d);
            ab = thour_b * 100 + tens_b * 10 + ones_b;
          } else {
            const max_d = maxDigitSafe(b);
            const min_d_carry = minDigitCarry(b);
            ...
            const thour = this.randInt(1, max_d);
            const tens = this.randInt(1, max_d);
            const ones = this.randInt(min_d_carry, 9);
            a = thour * 100 + tens * 10 + ones;

            const thour_b = this.randInt(1, max_d);
            const tens_b = this.randInt(min_d_carry, 9);
            const ones_b = this.randInt(min_d_carry, 9);
            ab = thour_b * 100 + tens_b * 10 + ones_b;
          }
          mq = { a, b, ans: a * b };
          dq = { ab, b, ans: ab * b };
          ...
          if (i < 10) {
            ...
            const thour = this.randInt(min_d_carry, 9);
            const tens = this.randInt(1, max_d);
            const ones = this.randInt(1, max_d);
            a = thour * 100 + tens * 10 + ones;

            const thour_b = this.randInt(min_d_carry, 9);
            const tens_b = this.randInt(min_d_carry, 9);
            const ones_b = this.randInt(1, max_d);
            ab = thour_b * 100 + tens_b * 10 + ones_b;
          } else {
          ...
          const thour = this.randInt(1, 9);
          const tens = this.randInt(1, 9);
          const ones = this.randInt(1, 9);
          a = thour * 100 + tens * 10 + ones;

          const thour_b = this.randInt(1, 9);
          const tens_b = this.randInt(1, 9);
          const ones_b = this.randInt(1, 9);
          ab = thour_b * 100 + tens_b * 10 + ones_b;
...
.answers-matrix .header-page > span:nth-last-child(1) {
  text-align: right;
  font-size: 17px;
}
.header-page > span:nth-last-child(2) img {
  height: 70px;
}
.num {
  display: flex;
