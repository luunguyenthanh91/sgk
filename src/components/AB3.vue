  ...
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 3</span>
    ...
    >
      ...
      <main class="sheet-grid" v-if="chunk">
        ...
        <table class="answers-table matrix">
          ...
          <tbody>
            <tr v-for="qId in 20" :key="'row-left-' + qId">
        ...
        <table class="answers-table matrix">
          ...
          <tbody>
            <tr v-for="qId in 20" :key="'row-right-' + qId">
              <td class="q-index">{{ qId }}</td>
              <td
...
export default {
    ...
    try {
      ...
      if (loaded && Array.isArray(loaded)) {
        ...
        for (let page = 0; page < 22; page++) {
          // 5 câu hundreds
          for (let i = 0; i < 5; i++) {
            const hundredsSet = this.genEightUnitsWith2Negatives();
            items.push({
          ...
          for (let i = 0; i < 15; i++) {
            const tens2 = this.genAddSubTwoTens();
            items.push(tens2);
          }
        }
        for (let page = 0; page < 22; page++) {
          ...
          for (let i = 0; i < 15; i++) {
            const tens2 = this.genAddSubThreeTens();
            items.push(tens2);
          }
        }
        ...
        for (let page = 0; page < 23; page++) {
          // 5 câu hundreds
          for (let i = 0; i < 5; i++) {
            const hundredsSet = this.genNineUnitsWith3Negatives();
            items.push({
              numbers: hundredsSet.numbers,
              sum: hundredsSet.sum,
              type: 6,
        ...
        this.groups = this.nestPages(items, 20, 5);
        this.saveJsonForBrowserDownload(this.groups, "AB3.json");
      }
    } catch (err) {
      const items = [];
  ...
  methods: {
    saveJsonForBrowserDownload(obj, filename = "AB3.json") {
      try {
    ...
    genNineUnitsWith3Negatives() {
      const count = 9;
      const negCount = 3;
      const MAX_TRIES = 2000;

      for (let t = 0; t < MAX_TRIES; t++) {
        // tạo pattern dấu: 3 dấu âm ngẫu nhiên, số đầu dương
        ...
        for (let i = 0; i < count; i++) {
          const sign = signs[i];
          let abs = this.randAbs1to9(); // 1–9
          let v = sign > 0 ? abs : -abs;

          // kiểm tra từng bước không âm
          if (sum + v <= 0) {
            if (sign > 0) {
              v = Math.max(1, 1 - sum + Math.ceil(Math.random() * 3));
            } else {
              // giảm biên độ âm để không làm âm tổng
              v = -Math.max(1, Math.min(abs, sum - 1));
              if (sum + v <= 0) {
                ok = false;
                break;
              }
        ...
        // kiểm tra hợp lệ
        if (ok && sum > 10 && this.okPrefixPositive(nums)) {
          return { numbers: nums, sum, format: "int" };
        }
      }

      // fallback an toàn (rất hiếm khi cần)
      const safe = [5, -2, 3, -1, 4, -3, 2, 1, 3];
      return { numbers: safe, sum: safe.reduce((a, b) => a + b, 0), format: "int" };
    },
    ...
    genAddSubThreeTens() {
      ...
      for (let t = 0; t < MAX_TRIES; t++) {
        // Ba số hàng chục
        const a = this.randIntInRangeExcludeZero(MIN, MAX);
        const b = this.randIntInRangeExcludeZero(MIN, MAX);
        const c = this.randIntInRangeExcludeZero(MIN, MAX);

        // Ngẫu nhiên chọn pattern dấu (luôn dương đầu tiên)
        // Có thể là [+,+,+], [+,+,-], [+,-,+], [+,-,-]
        const patterns = [
          [+1, +1, +1],
          [+1, +1, -1],
        ...
        const signs = this.shuffle(patterns)[0];

        // Áp dụng dấu
        const nums = [a * signs[0], b * signs[1], c * signs[2]];

        // Kiểm tra prefix dương
        let prefix = 0;
        let prefixOK = true;
        for (const v of nums) {
          prefix += v;
        ...
        // Điều kiện hợp lệ
        if (prefixOK && sum >= 10 && sum <= 999) {
          return {
            numbers: nums,
            sum,
            format: "int",
            type: "three-tens",
            label: "Cộng Trừ 3 con số hàng chục\nĐáp án: số hàng chục, hàng trăm",
          };
        }
      }

      // fallback nếu thử nhiều lần không ra
      return {
        numbers: [40, -20, 30],
        sum: 50,
        format: "int",
    ...
    genAddSubTwoTens() {
      let a = this.randIntInRangeExcludeZero(10, 99);
      let b = this.randIntInRangeExcludeZero(10, 99);

      // Chọn phép cộng hoặc trừ ngẫu nhiên
      let isAdd = Math.random() < 0.5;
      let sum = isAdd ? a + b : a - b;

      // Nếu trừ mà kết quả < 10 thì đổi sang phép cộng để đảm bảo dương và >=10
      if (sum < 10) {
        isAdd = true;
        sum = a + b;
      }

      // Nếu tổng vượt 999 thì giảm b xuống để kết quả vẫn trong hàng trăm
      while (sum > 999) {
        b = Math.floor(b / 2);
        sum = isAdd ? a + b : a - b;
      }
      ...
      return {
        numbers: isAdd ? [a, b] : [a, -b],
        sum,
        format: "int",
        type: "two-tens",
        label: "Cộng Trừ 2 con số hàng chục\nĐáp án: số hàng chục, hàng trăm",
      };
    },

    genEightUnitsWith2Negatives() {
      const count = 8;
      const signs = this.buildSignsFlexible(count, 2, {
        excludeFirst: true,
        forbidAdjacent: false, // cho phép âm liền nhau nếu cần
      });
      ...
      for (let t = 0; t < MAX_TRIES; t++) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          // Giữ prefix-sum luôn > 0
          if (sum + v <= 0) {
            if (sign > 0) {
              v = Math.max(1, 1 - sum + Math.ceil(Math.random() * 3));
            } else {
              // âm mà làm tổng <= 0 → giảm bớt độ lớn
              v = -Math.max(1, Math.min(abs, sum - 1));
              if (sum + v <= 0) {
                ok = false;
                break;
      ...
      // fallback nếu thử quá nhiều mà chưa ra
      const safe = [5, 4, -2, 3, -1, 2, 1, 1];
      return { numbers: safe, sum: safe.reduce((a, b) => a + b, 0), format: "int" };
    },
    ...
    genSequence({ count, scale, negCount, zeroSlots = 0 }) {
      ...
      // domain abs-drawers
      const drawAbs = () => {
        if (scale === "one-tens") return this.randIntInRangeExcludeZero(1, 9);
    ...
    buildIntegerAlternating(signs, drawAbs) {
      ...
      for (let t = 0; t < MAX_TRIES; t++) {
        ...
        if (ok && s > 0) {
          const invalid = nums.some((n) => n < 0 && Math.abs(n) < 100);
          if (!invalid) {
