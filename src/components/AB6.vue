  ...
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 6</span>
...
export default {
    ...
    try {
      ...
      if (loaded && Array.isArray(loaded)) {
        ...
        for (let page = 0; page < 22; page++) {
          // 5 câu tens
          for (let i = 0; i < 5; i++) {
            const tensSet = this.genSequence({ count: 10, scale: "tens", negCount: 3 });
            items.push({
              numbers: tensSet.numbers,
              sum: tensSet.sum,
              type: 6,
              place: "tens",
              format: tensSet.format,
            });
          }
          ...
          // 5 câu decimal unit
          for (let i = 0; i < 5; i++) {
            const decUnitSet = this.genSequence({
              count: 3,
              scale: "dec-unit",
              negCount: 1,
            });
            items.push({
              numbers: decUnitSet.numbers,
              sum: decUnitSet.sum,
              type: 6,
              place: "dec",
              format: decUnitSet.format,
            });
          }
        }
        ...
        this.saveJsonForBrowserDownload(items, "AB6.json");
      }
    } catch (err) {
      for (let page = 0; page < 22; page++) {
        // 5 câu tens
  ...
  methods: {
    saveJsonForBrowserDownload(obj, filename = "AB6.json") {
      try {
    ...
    pickNonAdjacentIndices(
      ...
      const okWithSet = (idx, picked) => {
        for (const p of picked) if (Math.abs(p - idx) <= minGap) return false; // tránh liền nhau
    ...
    buildSignsWithPlusesRandom(total = 10, plusCount = 3) {
      // tất cả âm, random đúng plusCount vị trí thành dương (bắt đầu dương)
      const signs = Array(total).fill(-1);
      signs[0] = +1;
      const need = Math.max(0, plusCount - 1);
     ...
     */
    genSequence({ count, scale, negCount }) {
      if (!Number.isInteger(count) || count < 1) count = 10;
      if (!Number.isInteger(negCount)) negCount = scale === "tens" ? 3 : 1;

      // không bắt buộc xen kẽ — chỉ random vị trí âm, bắt đầu dương, tránh 2 âm liền nhau
      ...
      if (isDecimal) {
        const built = this.buildDecimalAlternating(signs, drawAbs);
     ...
     *  - Cuối cùng tổng > 0
     */
    buildDecimalAlternating(signs, drawAbsCents) {
      ...
      for (let t = 0; t < MAX_TRIES; t++) {
        ...
        for (let i = 0; i < signs.length; i++) {
          const sign = signs[i];
          let abs = drawAbsCents(); // abs in cents, exclude 0

          // Ép điều kiện "không âm phần thập phân" khi bước là trừ:
          // nghĩa là khi trừ, phần lẻ của số trừ phải <= phần lẻ hiện tại
          if (sign < 0) {
            const fracNow = s % 100;
            let trials = 0;
            // nếu s quá nhỏ hoặc fracNow nhỏ, thử rút abs sao cho abs%100 <= fracNow
            ...
            // Cộng: luôn an toàn, nhưng để chuẩn bị cho âm kế tiếp,
            // nếu âm gần kề thì tăng nhẹ buffer
            const negSoon = i + 1 < signs.length && signs[i + 1] < 0;
            if (negSoon && abs < 20) abs = 20; // +0.20 buffer
            const v = abs;
            s += v;
            if (s <= 0) {
              ok = false;
        ...
        if (ok && s > 0) {
          // chuyển cents -> số thập phân JS (2 chữ số)
      ...
      // Fallback an toàn: toàn cộng nhỏ (0.20) để đảm bảo dương
      const safe = signs.map((sign) => (sign > 0 ? 20 : 10)); // +0.20, -0.10 (không mượn phần thập phân)
      let s = safe.reduce((a, b) => a + b, 0);
      if (s <= 0) s += 50;
      const numbers = safe.map((c) => this.centsToNumber(c));
      const sum = this.centsToNumber(s);
      return { numbers, sum };
    },
    ...
    genTensTwoNegAlt(countNums) {
      ...
      for (let i = 0; i < countNums; i++) {
        const sign = signs[i];
        let abs = this.randTenAbs10to99();

        if (sign > 0) {
          // nếu s thấp, bơm chút buffer để chịu được âm kế tiếp
    ...
    genUnitsTenNumbersWith3Pluses(target) {
      const signs = this.buildSignsWithPlusesRandom(10, 3);
...
.sheet-grid {
  display: grid;
  grid-template-rows: 1.5fr 0.8fr 0.8fr; /* Hàng 1 cao hơn */
  gap: 30px;
  flex: 1;
  width: 100%;
}
