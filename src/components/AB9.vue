  ...
  <div ref="exportRoot">
    <section class="page" v-for="(pg, idx) in groups" :key="idx">
      <div class="header-page">
        <span class="title-sgk">Level 9</span>
    ...
    >
      ...
      <main class="sheet-grid">
        ...
        <table class="answers-table matrix sum-table">
          ...
          <tbody>
            <tr v-for="row in 15" :key="'bot-r-' + row">
              <td class="q-index">{{ row }}</td>
              <td
...
export default {
  ...
  async mounted() {
    const jsonPath = "/AB9.json";

    try {
      const loaded = await this.tryLoadJson(jsonPath);

      // --- CASE: load saved pages (exported groups) ---
      if (loaded && Array.isArray(loaded)) {
        // Gán trực tiếp (cần đảm bảo cấu trúc đúng: array of pages)
        this.groups = loaded;

        // Nếu có các trang muldiv, rebuild mulAnsMap / divAnsMap từ dữ liệu đã lưu
        for (const pg of this.groups) {
          if (
            ...
            const divMap = {};
            pg.mul.forEach((m, i) => {
              // hỗ trợ cả trường hợp m chứa ans hoặc chỉ a/b
              mulMap[i + 1] = m.ans ?? Number(m.a) * Number(m.b);
            });
            pg.div.forEach((d, i) => {
              divMap[i + 1] = d.ans ?? Math.floor(Number(d.dividend) / Number(d.divisor));
            });
            // gán giữ reactivity
        ...
        // --- CASE: không có JSON hợp lệ -> sinh mới (giữ nguyên logic tạo trang chẵn/lẻ của bạn) ---
        const TOTAL_PAGES = 64; // ví dụ — bạn có thể đổi
        ...
        // tạo itemsEven (mỗi trang chẵn gồm 15 câu: 5+5+5) — giữ nguyên cách bạn đã viết
        for (let page = 0; page < EVEN_COUNT; page++) {
          // 5 câu hundreds
          for (let k = 0; k < 5; k++) {
            const s = this.genSequence({ count: 7, scale: "hundreds", negCount: 3 });
            itemsEven.push({
              numbers: s.numbers,
              sum: s.sum,
              type: 6,
          ...
          // 5 câu thousands
          for (let k = 0; k < 5; k++) {
            const s = this.genSequence({ count: 5, scale: "thousands", negCount: 2 });
            itemsEven.push({
              numbers: s.numbers,
              sum: s.sum,
              type: 6,
        ...
        // --- Lưu groups đã sinh ra thành AB9.json ---
        try {
          // lưu đúng cấu trúc: array of pages (this.groups)
          this.saveJsonForBrowserDownload(this.groups, "AB9.json");
        } catch (saveErr) {
          // nếu việc lưu thất bại thì hiển thị console nhưng không dừng app
          console.error("Failed to auto-save AB9.json:", saveErr);
        }

        // NOTE: khi mới sinh, makeMulDivPage đã gán mulAnsMap/divAnsMap bên trong nó
        // nhưng nếu bạn muốn rebuild cho an toàn, làm lại ở đây:
        for (const pg of this.groups) {
          if (
            pg &&
            pg.mode === "muldiv40" &&
      ...
      // nếu fetch/json lỗi -> fallback: sinh mới giống nhánh else và lưu
      console.error("Error loading AB9.json, will generate new pages:", err);
      ...
      try {
        ...
        console.error("Failed to auto-save AB9.json after catch:", saveErr);
      }

      // rebuild maps cho muldiv pages
      for (const pg of this.groups) {
        if (
          pg &&
          pg.mode === "muldiv40" &&
    ...
    // cuối cùng: tạo trang đáp án tổng hợp (dùng flatQuestions của bạn)
    this.answerPages = this.makeAnswerPages(3);
  },

  methods: {
    saveJsonForBrowserDownload(obj, filename = "AB9.json") {
      try {
    ...
    genDivQuestion() {
      ...
      if (pick === 0) {
        ...
        const minDividend = 1000,
          maxDividend = 9999;
        let qMin = Math.ceil(minDividend / divisor);
        let qMax = Math.floor(maxDividend / divisor);
        qMin = Math.max(qMin, 2);
        if (qMax < qMin) return this.genDivQuestion(); // thử lại nếu biên xấu
      ...
      if (pick === 1) {
        // chục nghìn ÷ chục
        const divisor = this.randInt(10, 99);
        const minDividend = 10000,
          maxDividend = 99999;
        let qMin = Math.ceil(minDividend / divisor);
        let qMax = Math.floor(maxDividend / divisor);
        qMin = Math.max(qMin, 2);
        if (qMax < qMin) return this.genDivQuestion();
        const quotient = this.randInt(qMin, qMax);
        const dividend = divisor * quotient;
        return { dividend, divisor, ans: quotient };
      }

      // pick === 2: chục nghìn ÷ trăm
      {
