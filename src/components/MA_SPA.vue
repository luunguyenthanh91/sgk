  ...
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level Pro A</span>
...
export default {
  ...
  mounted() {
    ...
    for (let page = 0; page < 67; page++) {
      ...
      // 5 câu decimal unit
      for (let i = 0; i < 5; i++) {
        const decUnitSet = this.generateTenDecimal_Mix_3Minus_FractCheck();
        items.push({
  ...
  methods: {
    generateTenDecimal_Mix_3Minus_FractCheck() {
      const drawSmall = () => Math.floor(Math.random() * 999) + 1; // 0.01–9.99
      const drawBig = () => Math.floor(Math.random() * 8999) + 1001; // 10.01–99.99

      // eslint-disable-next-line no-constant-condition
      while (true) {
        const smallNums = Array.from({ length: 5 }, drawSmall);
        const bigNums = Array.from({ length: 5 }, drawBig);
        const merged = [...smallNums, ...bigNums];

        const shuffled = this.shuffle(merged);

        const validIdx = [1, 2, 3, 4, 5, 6, 7, 8, 9];
        const negIdx = this.shuffle(validIdx).slice(0, 3);

        const values = shuffled.map((v, i) => (negIdx.includes(i) ? -v : v));
        ...
        for (const v of values) {
          ...
          // Điều kiện 1: tổng số phải dương
          if (sum <= 0) {
            valid = false;
            break;
          }

          // Điều kiện 2: phần thập phân riêng cũng dương
          const frac = sum % 100; // cents còn lại
          if (frac <= 0) {
            valid = false;
            break;
          }
        ...
        if (valid && sum > 0) {
          return {
            numbers: values.map(this.centsToNumber),
            sum: this.centsToNumber(sum),
          };
        }
      }
    },
    ...
    generateCustomDecimalUnit8() {
      const drawSmall = () => Math.floor(Math.random() * 99) + 1; // 0.01 → 0.99
      const drawBig = () => Math.floor(Math.random() * 900) + 100; // 1.00 → 9.99

      // eslint-disable-next-line no-constant-condition
      while (true) {
        // 1. Random vị trí cho 3 số nhỏ trong 8
        const positions = this.shuffle([0, 1, 2, 3, 4, 5, 6, 7]);
        const smallIdx = positions.slice(0, 3);
        const nums = [];

        for (let i = 0; i < 8; i++) {
          const cents = smallIdx.includes(i) ? drawSmall() : drawBig(); // cents = 1..99 or 100..999
          nums.push(cents);
        }

        // 2. Chọn ngẫu nhiên 3 vị trí làm số âm (trừ vị trí 0)
        const signIdxPool = [1, 2, 3, 4, 5, 6, 7];
        const negIdx = this.shuffle(signIdxPool).slice(0, 3);

        const values = nums.map((c, i) => (negIdx.includes(i) ? -c : c));

        // 3. Kiểm tra tổng từng bước phải dương
        let sum = 0;
        let valid = true;
        for (const v of values) {
          sum += v;
        ...
        if (valid && sum > 0) {
          return {
            numbers: values.map(this.centsToNumber), // → decimal format
            sum: this.centsToNumber(sum),
          };
        }
      }
