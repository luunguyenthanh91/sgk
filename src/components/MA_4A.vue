  ...
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 4A</span>
...
export default {
  ...
  mounted() {
    ...
    for (let page = 0; page < 67; page++) {
      ...
      // 5 câu decimal unit
      for (let i = 0; i < 5; i++) {
        const q = this.generateOneTensThreeUnitsWith1Minus();
        items.push(q);
      }
    }
  ...
  methods: {
    ...
    generateOneTensThreeUnitsWith1Minus() {
      ...
      for (let t = 0; t < MAX_TRIES; t++) {
        // 1 số hàng chục (10–99) + 3 số đơn vị (1–9)
        const tens = Math.floor(Math.random() * 90) + 10; // 10..99
        const units = Array.from({ length: 3 }, () => Math.floor(Math.random() * 9) + 1); // 1..9

        // Trộn để vị trí hàng chục random
        const base = this.shuffle([tens, ...units]);

        // Thử gán 1 số âm ở vị trí ngẫu nhiên (có thể là tens hoặc unit)
        // Nếu âm ngay đầu sẽ thường fail prefix-sum, nhưng ta để validator xử lý.
        for (let trySign = 0; trySign < 6; trySign++) {
          const arr = base.slice();
          const negIdx = Math.floor(Math.random() * 4); // 0..3
          arr[negIdx] = -arr[negIdx];

          // Kiểm tra mọi bước dương và tổng cuối > 10
          let s = 0,
            ok = true;
          for (const v of arr) {
            s += v;
      ...
      return this.generateOneTensThreeUnitsWith1Minus();
    },
