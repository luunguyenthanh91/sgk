  ...
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 9B</span>
...
export default {
  ...
  methods: {
    generateCustomDecimalUnit8() {
      const drawSmall = () => Math.floor(Math.random() * 99) + 1; // 0.01..0.99  (1..99 cents)
      const drawBig = () => Math.floor(Math.random() * 899) + 101; // 1.01..9.99 (101..999 cents)
      const MAX_TRIES = 4000;

      for (let t = 0; t < MAX_TRIES; t++) {
        // 1) Xếp ngẫu nhiên 5 small + 5 big cho 10 vị trí
        ...
        // 2) Chọn 3 vị trí âm ngẫu nhiên, nhưng không chọn index 0
        const negIdx = this.shuffle([1, 2, 3, 4, 5, 6, 7, 8, 9]).slice(0, 3);
        const isNeg = (i) => negIdx.includes(i);

        let s = 0; // tổng theo cents
        const vals = []; // mảng cents với dấu
        let ok = true;
        ...
        for (let i = 0; i < 10 && ok; i++) {
          ...
          if (isNeg(i)) {
            // Bước âm: yêu cầu s hiện tại đủ lớn & phần thập phân sau phép vẫn dương
            if (s <= 1) {
              ok = false;
              break;
            }
            ...
            let abs = 0,
              tries = 0;
            while (tries++ < 400) {
              const cand = k === "small" ? drawSmall() : drawBig();
              // 1) Không vượt quá s để prefix vẫn dương
              // 2) (cand % 100) < (fracNow) để phần thập phân sau phép trừ vẫn dương (không mượn, không bằng 0)
              if (cand < s && cand % 100 < fracNow) {
                abs = cand;
                break;
              }
            }
            ...
            // Bước dương: chỉ cần giữ prefix > 0, thêm mẹo tránh để ngay trước bước âm có fracNow==0
            let abs = k === "small" ? drawSmall() : drawBig();

            // Nếu bước tiếp theo là âm, cố gắng tránh làm (s + abs) % 100 == 0
            const negSoon = i + 1 < 10 && isNeg(i + 1);
            if (negSoon && (s + abs) % 100 === 0) {
              let t2 = 0;
              while (t2++ < 50) {
                const tryAbs = k === "small" ? drawSmall() : drawBig();
                if ((s + tryAbs) % 100 !== 0) {
                  abs = tryAbs;
                  break;
                }
        ...
        if (ok && s > 0) {
          return {
            numbers: vals.map(this.centsToNumber), // chuyển về số thập phân
            sum: this.centsToNumber(s),
            type: "dec-unit-10-3neg",
            place: "dec-unit",
            format: "decimal",
          };
        }
      ...
      // Fallback cực hiếm: thử lại
      return this.generateCustomDecimalUnit8();
    },
     ...
     */
    // thêm tham số minSum, maxSum (tuỳ chọn)
    genSequence({
      count,
      scale,
      negCount,
      ...
      const drawAbs = () => {
        ...
        if (scale === "dec-unit") return this.randCentsInRange(1, 999);
        if (scale === "dec-tens") return this.randCentsInRange(1001, 9999);
        return this.randIntInRangeExcludeZero(1, 9);
      };

      const isDecimal = scale === "dec-unit" || scale === "dec-tens";
      if (isDecimal) {
        const built = this.buildDecimalAlternating(signs, drawAbs, scale, {
          zeroSlots,
          minSum,
          maxSum,
        });
      ...
      const built = this.buildIntegerAlternating(signs, drawAbs, { minSum, maxSum });
    ...
    buildIntegerAlternating(signs, drawAbs, { minSum = 0, maxSum = Infinity } = {}) {
      const MAX_TRIES = 4000;
      for (let t = 0; t < MAX_TRIES; t++) {
        const nums = [];
        let s = 0;
    ...
    buildDecimalAlternating(signs, drawAbsCents, scale, options = {}) {
      const { zeroSlots = 0, minSum = 0, maxSum = Infinity } = options;
      const MIN_SUM_CENTS = Math.round(minSum * 100);
      const MAX_SUM_CENTS = isFinite(maxSum) ? Math.round(maxSum * 100) : Infinity;
      ...
      for (let t = 0; t < MAX_TRIES; t++) {
        ...
        if (ok && s > MIN_SUM_CENTS && s <= MAX_SUM_CENTS && zerosLeft === 0) {
      ...
      const safe = signs.map((sgn, idx) => {
        if (scale === "dec-unit")
          return sgn > 0 ? 150 : idx % 2 === 1 && zeroSlots > 0 ? 30 : 20;
        return sgn > 0 ? 20 : 10;
      });
