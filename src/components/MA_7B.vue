  ...
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 7B</span>
...
export default {
  ...
  mounted() {
    ...
    for (let page = 0; page < 67; page++) {
      // 5 câu hundreds
      for (let i = 0; i < 5; i++) {
        const q = this.generateFourTensFourUnitsWith3Minus();
        items.push(q);
      }

      for (let i = 0; i < 10; i++) {
        const q = this.generateSixTensWith2Minus();
        items.push(q);
      }
    }
  ...
  methods: {
    generateSixTensWith2Minus() {
      const MAX_TRIES = 3000;

      const randTen = () => Math.floor(Math.random() * 90) + 10; // 10–99

      for (let t = 0; t < MAX_TRIES; t++) {
        // 1. Tạo 6 số từ 10–99
        const base = Array.from({ length: 6 }, randTen);

        // 2. Chọn 2 vị trí để đặt số âm (trừ vị trí 0)
        const negIdx = this.shuffle([1, 2, 3, 4, 5]).slice(0, 2);

        // 3. Gán dấu âm theo vị trí
        const arr = base.map((v, i) => (negIdx.includes(i) ? -v : v));

        // 4. Kiểm tra từng bước > 0 và tổng trong khoảng yêu cầu
        let sum = 0;
        let ok = true;
      ...
      // fallback
      return this.generateSixTensWith2Minus();
    },
    generateFourTensFourUnitsWith3Minus() {
      const MAX_TRIES = 4000;

      const randTen = () => Math.floor(Math.random() * 90) + 10; // 10–99
      const randUnit = () => Math.floor(Math.random() * 9) + 1; // 1–9

      for (let t = 0; t < MAX_TRIES; t++) {
        // 1. Tạo 4 số hàng chục và 4 số đơn vị
        const tens = Array.from({ length: 4 }, randTen);
        const units = Array.from({ length: 4 }, randUnit);

        // 2. Trộn lại để vị trí random
        const base = this.shuffle([...tens, ...units]);

        // 3. Chọn 3 vị trí ngẫu nhiên để âm (trừ vị trí 0)
        const negIdx = this.shuffle([1, 2, 3, 4, 5, 6, 7]).slice(0, 3);

        // 4. Gán dấu và kiểm tra prefix sum
        const arr = base.map((v, i) => (negIdx.includes(i) ? -v : v));
        ...
        if (ok && sum > 10) {
          return {
            numbers: arr,
            sum,
            type: "4tens_4units_3minus",
            place: "mixed",
          };
        }
      }

      // fallback: gọi lại chính mình
      return this.generateFourTensFourUnitsWith3Minus();
    },
