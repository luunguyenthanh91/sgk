  ...
  <div ref="exportRoot">
    <section class="page" v-for="(pg, idx) in groups" :key="idx">
      <div class="header-page">
        <span class="title-sgk">Level 8</span>
...
export default {
  ...
  async mounted() {
    const jsonPath = "/AB8.json";

    try {
      const loaded = await this.tryLoadJson(jsonPath);
      if (loaded && Array.isArray(loaded)) {
        this.groups = loaded;
        // --- normalize & rebuild answers after loading groups ---
        let pages = null;

        // nếu loaded là mảng trang
        if (Array.isArray(loaded)) pages = loaded;
        // nếu loaded là object chứa groups / items
        else if (loaded && Array.isArray(loaded.groups)) pages = loaded.groups;
        else if (loaded && Array.isArray(loaded.items)) pages = loaded.items;
        // fallback: tìm bất kỳ mảng page-like trong object
        else if (loaded && typeof loaded === "object") {
          const cand = [];
          for (const v of Object.values(loaded)) {
            if (Array.isArray(v) && v.length && typeof v[0] === "object") {
              // heuristic: page-like object có pageIndex hoặc mode hoặc items
              if (v[0].pageIndex || v[0].mode || v[0].items) cand.push(...v);
            }
          }
          if (cand.length) pages = cand;
        }

        if (Array.isArray(pages) && pages.length) {
          // đảm bảo pageIndex cho từng page nếu thiếu
          pages = pages.map((p, i) => ({
            ...(p || {}),
            pageIndex: Number(p?.pageIndex ?? i + 1),
          }));

          // gán groups (clone sâu để Vue nhận reactivity)
          this.groups = JSON.parse(JSON.stringify(pages));

          // rebuild mul/div answer maps từ các trang muldiv40
          const newMul = { ...(this.mulAnsMap || {}) };
          const newDiv = { ...(this.divAnsMap || {}) };

          for (const pg of this.groups) {
            if (pg && pg.mode === "muldiv40") {
              const idx = Number(pg.pageIndex);
              const mulMap = {};
              const divMap = {};

              if (Array.isArray(pg.mul)) {
                pg.mul.forEach((m, i) => {
                  // dùng m.ans nếu có, ngược lại tính a*b
                  const ans =
                    m && typeof m.ans !== "undefined"
                      ? m.ans
                      : Number(m?.a || 0) * Number(m?.b || 0);
                  mulMap[i + 1] = ans;
                });
              }

              if (Array.isArray(pg.div)) {
                pg.div.forEach((d, i) => {
                  // dùng d.ans nếu có, ngược lại tính dividend/divisor (chia hết theo logic generator)
                  const ans =
                    d && typeof d.ans !== "undefined"
                      ? d.ans
                      : d && Number(d.divisor)
                      ? Math.floor(Number(d.dividend) / Number(d.divisor))
                      : "";
                  divMap[i + 1] = ans;
                });
              }
          ...
          // dùng gán mới để Vue detect thay đổi
          this.mulAnsMap = { ...newMul };
          this.divAnsMap = { ...newDiv };

          // tạo lại answerPages (hàm dùng this.groups nội tại)
          this.answerPages = this.makeAnswerPages(3);
        } else {
          // nếu không đọc đc pages hợp lệ thì giữ nguyên behaviour cũ (không ghi đè)
          console.warn(
            "Loaded JSON không chứa pages hợp lệ — bỏ qua bước rebuild answers."
          );
        }
      } else {
        ...
        for (let page = 0; page < EVEN_COUNT; page++) {
          // 5 câu hundreds
          for (let k = 0; k < 5; k++) {
            const s = this.genSequence({ count: 6, scale: "hundreds", negCount: 2 });
            itemsEven.push({
              numbers: s.numbers,
              sum: s.sum,
              type: 6,
          ...
          // 5 câu thousands
          for (let k = 0; k < 5; k++) {
            const s = this.genSequence({ count: 4, scale: "thousands", negCount: 1 });
            itemsEven.push({
              numbers: s.numbers,
              sum: s.sum,
              type: 6,
        ...
        this.groups = merged;
        this.saveJsonForBrowserDownload(this.groups, "AB8.json");
  ...
  methods: {
    saveJsonForBrowserDownload(obj, filename = "AB8.json") {
      try {
