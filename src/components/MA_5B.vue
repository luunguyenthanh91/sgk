  ...
  <div ref="exportRoot">
    <section class="page" v-for="(questions, qsIdx) in groups" :key="qsIdx">
      <div class="header-page">
        <span class="title-sgk">Level 5B</span>
...
export default {
  ...
  mounted() {
    ...
    for (let page = 0; page < 67; page++) {
      ...
      // 5 câu decimal unit
      for (let i = 0; i < 5; i++) {
        const q = this.generateTwoTensThreeUnitsWith2Minus();
        items.push(q);
      }
    }
  ...
  methods: {
    generateTwoTensThreeUnitsWith2Minus() {
      ...
      for (let t = 0; t < MAX_TRIES; t++) {
        // 1. Tạo 2 số hàng chục và 3 số đơn vị
        const tens = Array.from({ length: 2 }, randTen);
        const units = Array.from({ length: 3 }, randUnit);

        // 2. Gộp lại và trộn vị trí
        const base = this.shuffle([...tens, ...units]);

        // 3. Chọn 2 vị trí để đặt số âm (tránh index 0)
        const negIdx = this.shuffle([1, 2, 3, 4]).slice(0, 2);
        const arr = base.map((v, i) => (negIdx.includes(i) ? -v : v));

        // 4. Kiểm tra prefix-sum và tổng
        let sum = 0;
        let valid = true;
        for (const v of arr) {
          sum += v;
        ...
        if (valid && sum > 10 && sum <= 999) {
          return {
            numbers: arr,
            sum,
            type: "2tens_3units_2minus",
            place: "mixed",
          };
        }
      }
      ...
      // fallback
      return this.generateTwoTensThreeUnitsWith2Minus();
    },
